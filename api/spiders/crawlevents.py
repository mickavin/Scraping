...
class CrawleventsSpider(CrawlSpider):
    ...
    crawlera_apikey = '7f4f49d98cbe4bd8a6fd1fa8ff05b39a'
    ...
    def start_requests(self):
        for url in self.start_urls:
            yield Request(url=url, callback=self.parse_begin)
    ...
    def parse_begin(self, response):
        links = response.xpath('//div[@class="resultat-vide-prop-first"]/ul/li/a/@href').extract()
        for link in links:
            yield SplashRequest(
                ...
                meta={'url': response.urljoin(link),
                        'http_method': 'POST',
                        'wait': 3,
